  ...
  async function handleExportCostCentersXlsx() {
    ...
    try {
      ...
      const materialsRows = materialData.rows.map((line) => ({
        ...
        'Fecha prevista': formatDateTime(line.scheduled_date),
        Albaran: line.picking_name || '—',
        Referencia: line.reference || '—',
        Proveedor: line.partner_name || '—',
      ...
      const attendancesRows = attendanceData.rows.map((row) => ({
        ID: row.id,
        'Cód. BIM': row.code_bim || '—',
        Puesto: row.job_name || '—',
        Empleado: row.employee_name || '—',
        'Proyecto entrada': row.project_name || '—',
        'Proyecto salida': row.project_out_name || '—',
        Entrada: formatDateTime(row.check_in),
        Salida: formatDateTime(row.check_out),
        DE: row.de || '—',
        DS: row.ds || '—',
        Almuerzo: row.almuerzo ? 'Sí' : 'No',
        Comida: row.comida ? 'Sí' : 'No',
        Festivo: row.festivo ? 'Sí' : 'No',
        Nota: row.note || '—',
        Horas: row.hours,
        'HH totales': row.total_hh,
        UD: row.ud,
        MOD: row.mod,
        'MOE (€)': row.moe,
        'MOF (€)': row.mof,
        'Horas extra': row.t_extra,
        'Dietas (€)': row.dietas,
      ...
      const partnerAttendancesRows = partnerAttendanceData.rows.map((row) => ({
        ...
        'Empresa partner': row.partner_parent || '—',
        Contrato: row.contract_name || '—',
        Proyecto: row.project_name || '—',
        Entrada: formatDateTime(row.check_in),
        Salida: formatDateTime(row.check_out),
        'Hora entrada contrato': row.contract_check_in_time,
        'Hora salida contrato': row.contract_check_out_time,
        'Entrada calculada': formatDateTime(row.check_in_calculated),
        'Salida calculada': formatDateTime(row.check_out_calculated),
        'Usar tiempo calculado': row.use_calculated_time ? 'Sí' : 'No',
        Horas: row.hours,
        'Tiempo total': row.tiempo_total,
        'Tiempo calculado': row.tiempo_total_calculado,
        Nota: row.note || '—',
        'EUR por hora': row.hour_cost,
        Total: row.total,
      }));
      ...
      const shipmentsRows = shipmentData.rows.map((row) => ({
        ...
        'KM inicial': row.km_initial,
        'KM final': row.km_final,
        'KM recorridos': row.km_final - row.km_initial,
        Cantidad: row.qty,
