...
const ALTA_REDIRECT_BASE = 'https://alta.soltecgrupo.es/auth/sso';
...
export async function POST(req: NextRequest) {
  ...
  if (!token) {
    console.warn('[Alta SSO] Petición sin token de autorización.');
    return Response.json({ success: false, error: 'No autenticado.' }, { status: 401 });
  }

  if (!PLANNER_JWT_SECRET) {
    console.error('[Alta SSO] PLANNER_JWT_SECRET no está configurado.');
  ...
  try {
    ...
    console.log('[Alta SSO] Respuesta Odoo /me:', JSON.stringify(odooJson?.result ?? odooJson?.error));

    const partner = odooJson?.result?.partner;
    if (!partner?.email) {
      console.warn('[Alta SSO] Email no encontrado en la respuesta de Odoo.');
    ...
    console.error('[Alta SSO] Error al contactar con Odoo:', err);
  ...
  console.log('[Alta SSO] Enviando claims:', JSON.stringify({ ...payload }));

  const jwt = signHS256(payload, PLANNER_JWT_SECRET);
  const redirectUrl = `${ALTA_REDIRECT_BASE}?t=${encodeURIComponent(jwt)}`;

  console.log(`[Alta SSO] Token generado (jti=${jti}, email=${email}), redirigiendo a ${ALTA_REDIRECT_BASE}`);
